...
void KeyMap::saveKeyMap(const QString &output)
{
     QJsonDocument jsonDoc;
     QMetaEnum metaEnumKey = QMetaEnum::fromType<Qt::Key>();
     ...
     // switchKey
     //rootObj["switchKey"] = QKeySequence((Qt::Key)m_switchKey).toString();
     rootObj["switchKey"] = metaEnumKey.valueToKey(m_switchKey);
     
     // mouseMoveMap
     rootObj["mouseMoveMap"] = m_mouseMoveMap.toQJsonObject();

     // keyMapNodes
     QJsonArray keyMapNodes;
     for (auto t : m_keyMapNodes) {
         QJsonObject tmp = t.toQJsonObject();
         if (!tmp.empty()) {
             keyMapNodes.push_back(tmp);
         } else {
             qDebug() << "serial empty" << t.type;
         }
     }
     rootObj["keyMapNodes"] = keyMapNodes;
